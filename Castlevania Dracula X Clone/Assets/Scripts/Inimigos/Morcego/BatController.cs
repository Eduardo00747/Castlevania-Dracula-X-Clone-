...
public class BatController : MonoBehaviour
{
    ...
    IEnumerator StartWavyMovement()
    {
        ...
        float amplitude = 1.5f; // Ajuste o valor da amplitude desejada
        float frequency = 2.5f; // Ajuste o valor da frequência desejada

        while (true)
        {
            timeElapsed += Time.deltaTime;
            float newY = originalY + Mathf.Sin(timeElapsed * frequency) * amplitude;
            Vector3 newPosition = new Vector3(transform.position.x + Time.deltaTime, newY, transform.position.z);
